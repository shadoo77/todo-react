...
router.post('/add',checkAuth, (req, res, next) => {
    ...
    const {error} = validateEntries(todoItem, null);
    if(error) return res.status(400).json({errorMessage: error.details[0].message});
    else {
        todoItem.user_id = req.userData.userId;
        const addItemQr = "INSERT INTO items (id, task, done, user_id) VALUES(?, ?, ?, ?)";
        db.query(addItemQr, [todoItem.id, todoItem.task, todoItem.done, todoItem.user_id], (err, results, fields) => {
            if(err) throw err;
...
router.put('/update/:id', checkAuth, (req, res, next) => {//
    ...
    const {error} = validateEntries(todoItem, 'update');
    if(error) return res.status(400).json({errorMessage: error.details[0].message});
